...
import { imageEndpoints, photoEndpoints } from "../apis";
...
export const createImageCategory = async (data, token) => {
  ...
  try {
    ...
    if (!response?.data?.success) {
      throw new Error(
        response?.data?.message || "Could Not Create Image Category"
      );
    }
...
export const editImageCategory = async (data, token) => {
  ...
  try {
    ...
    if (!response?.data?.success) {
      throw new Error(
        response?.data?.message || "Could Not Update Image Category"
      );
    }
...
export const deleteImageCategory = async (categoryId, token) => {
  ...
  try {
    ...
    const response = await apiConnector(
      "DELETE",
      `${DELETE_IMAGE_CATEGORY_API}/${categoryId}`,
      null,
      {
        Authorization: `Bearer ${token}`,
      }
    ...
    if (!response?.data?.success) {
      throw new Error(
        response?.data?.message || "Could Not Delete Image Category"
      );
    }
...
export const fetchImageCategoryDetails = async (categoryId) => {
  ...
  try {
    const response = await apiConnector(
      "GET",
      `${IMAGE_CATEGORYPAGEDETAILS_API}/${categoryId}`
    );
    if (!response?.data?.success) {
      throw new Error(
        response?.data?.message || "Could Not Fetch Image Category Details"
      );
    }
    result = response?.data?.data;
  } catch (error) {
