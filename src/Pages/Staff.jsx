        ...
        {Object.entries(groupedCategories).map(([branch, categories]) => (
          <div key={branch} className="mb-24 ">
            <div className="flex flex-col gap-5 items-center justify-center mb-10">
              <h2 className="text-2xl font-semibold  text-center font-m1 text-[45px] text-bhawaniDark">
                {branch} Staff
              </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {categories.map((category) => (
                ...
                >
                  ...
                  />
                  <p className="font-semibold font-m1 text-center text-[22px]">
                    {category.name}
                  </p>
                  <Link
                    to={`/staff/${category.name
                      ...
                      .toLowerCase()}`}
                  >
                    <RButton className="scale-0 text-[15px] font-verdana mt-3 group-hover:scale-100 transition-all duration-700">
                      {"Know Your Mentors"}
                    </RButton>
                  </Link>
                </div>
