...
function Table({ courseDetails, setCourseDetails, courseCat }) {
  ...
  const [selectCourseCategory, setSelectCourseCategory] = useState("All");
  const [searchTerm, setSearchTerm] = useState("");
  const navigate = useNavigate();
  const limit = 5;

  const categories = [{ _id: "All", name: "All Programs" }, ...courseCat];

  let filteredData =
    selectCourseCategory === "All"
      ? courseDetails
      : courseDetails.filter((item) =>
          item.categoryProgram.includes(selectCourseCategory)
        );
  ...
  const handleCourseCatChange = (e) => {
    setSelectCourseCategory(e.target.value);
    setPage(1);
  };
    ...
    <div className="flex flex-col bg-white w-[97%] h-auto rounded-xl shadow-xl border-gray-300 border-[1px] pt-5 pb-12 mt-20 mb-20">
      <div className="flex flex-row items-center justify-between pl-10 pr-16 pt-5">
        <div className="flex flex-col justify-start items-start">
          <div className="flex justify-center items-center gap-3">
            <div className="bg-bhawaniDark/10 p-3 rounded-md ">
              <RiGroupLine className="text-[20px] text-bhawaniDark" />
            </div>
            <p className="font-m2 text-3xl font-bold text-bhawaniDark">
              Course Catalog
            </p>
          </div>
        ...
        <div className="flex gap-4">
          ...
          <select
            ...
            onChange={handleCourseCatChange}
      ...
      <div className="p-10">
        {paginatedData.length > 0 ? (
          ...
          <p className="text-center ">No Course found for this filter.</p>
        )}
      </div>
