...
function Table({ photoDetails, setPhotoDetails, imageCat }) {
  ...
  const [selectImageCategory, setSelectImageCategory] = useState("All");
  const [searchTerm, setSearchTerm] = useState("");
  const navigate = useNavigate();
  const limit = 5;

  const categories = [{ _id: "All", name: "All Category" }, ...imageCat];

  let filteredData =
    selectImageCategory === "All"
      ? photoDetails
      : photoDetails.filter((item) =>
          item.imageCategory.includes(selectImageCategory)
        );
  ...
  const handleImageCatChange = (e) => {
    setSelectImageCategory(e.target.value);
    setPage(1);
  };
    ...
    <div className="flex flex-col bg-white w-[97%] h-auto rounded-xl shadow-xl border-gray-300 border-[1px] pt-5 pb-12 mt-20 mb-20">
      <div className="flex flex-row items-center justify-between pl-10 pr-16 pt-5">
        <div className="flex flex-col justify-start items-start">
          <div className="flex justify-center items-center gap-3">
            <div className="bg-gray-300 p-2 rounded-md ">
              <ImCamera className="text-[20px] text-bhawaniRed" />
            </div>
            <p className="font-m2 text-[30px] font-bold">Campus Life Posts</p>
          </div>
          <p className="text-gray-500">
            Showcase student stories through photos.
          </p>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <select
            value={selectImageCategory}
            onChange={handleImageCatChange}
      ...
      <div className="p-10">
        {paginatedData.length > 0 ? (
                  ...
                  <tr key={index} className="hover:bg-blue-50 transition">
                    ...
                    <td className="py-5">
                      <div className="inline-block text-[14px] bg-blue-50 border border-blue-500 text-blue-600 text-center py-[2px] px-3 rounded-2xl">
                        {item.imageCategory
                          .map(
                            (catId) =>
                              imageCat.find((cat) => cat._id === catId)?.name ||
                              "Unknown"
                          )
                          .join(", ")}
                      </div>
                    ...
                    <td className="px-4 py-5 flex gap-5">
                      <PiEye
                        ...
                        onClick={() =>
                          navigate(`/dashboard/photos/${item.imageCategory}`)
                        }
                      />
                      <FaRegEdit
                        className="text-yellow-500 text-[18px] cursor-pointer"
                        onClick={() =>
                          navigate(`/dashboard/photos/editPhoto?id=${item._id}`)
                        }
                      />
                      <DeletePhotos
                        photoId={item._id}
          ...
          <p className="text-center ">
            No Photos Details found for this filter.
          </p>
        )}
      </div>
